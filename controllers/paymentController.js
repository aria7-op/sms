...
// Create a proxy to automatically initialize Prisma before each operation
    ...
    try {
      ...
      upload.array('files', 5)(req, res, async (err) => {
        ...
        try {
          ...
          const student = payment.studentId ? await prismaProxy.student.findFirst({
            where: { id: payment.studentId },
            include: { user: { select: { firstName: true, lastName: true } } }
          }) : null;

          const parent = payment.parentId ? await prismaProxy.parent.findFirst({
          ...
          try {
            ...
            if (hasGoogleDriveTemplate) {
              ...
              try {
                ...
                const excelFileRecord = await prismaProxy.file.create({
                  data: {
  ...
  async getPayments(req, res) {
    try {
      ...
      const [paymentsResult, totalResult] = await Promise.all([
        ...
        prismaProxy.payment.count({ where })
      ]);
      ...
      try {
        ...
        console.error('Error during payment sanitization:', sanitizeError);
        // Return empty array if sanitization fails
        sanitizedPayments = [];
      }
