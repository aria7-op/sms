...
async function initializeDatabase() {
  try {
    ...
    const connectionOptions = {
      ...
      connectionLimit: 10,
      queueLimit: 0,
      acquireTimeout: 25000, // 25 seconds
      timeout: 25000, // 25 seconds
      reconnect: true,
      connectTimeout: 25000, // 25 seconds for initial connection
      acquireTimeoutMillis: 25000, // 25 seconds for acquiring connection
      timeoutMillis: 25000 // 25 seconds for query timeout
    };
...
app.use((req, res, next) => {
  ...
  res.header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, x-client-version, x-device-type, x-request-id, x-request-timestamp, x-minimal-data, x-no-compression, cache-control, pragma, expires, if-none-match, if-modified-since, accept, accept-encoding, accept-language, connection, host, referer, user-agent');
  res.header('Access-Control-Expose-Headers', 'x-response-size, cache-control, etag, last-modified, content-encoding, content-length');
...
async function query(sql, params = []) {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => reject(new Error('Database query timeout')), 25000); // 25 seconds
  });
...
app.get('/api/customers/conversion-analytics', async (req, res) => {
  try {
...
app.get('/api/customers/conversion-rates', async (req, res) => {
  try {
...
app.get('/api/customers/analytics/reports', async (req, res) => {
  try {
    if (!dbPool) {
      return res.json({
        success: true,
...
app.get('/api/customers/analytics/dashboard', async (req, res) => {
  try {
    if (!dbPool) {
      return res.json({
        success: true,
...
app.get('/api/customers/unconverted', async (req, res) => {
  try {
...
async function startServer() {
  try {
    ...
    // Set server timeout to 25 seconds
server.timeout = 25000; // 25 seconds
server.keepAliveTimeout = 25000; // 25 seconds
